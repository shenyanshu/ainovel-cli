...
**书已完结也适用**：即使全书已 `complete`，若用户要求重写整本书，仍走上面的确认门派 `architect_long` 做 `replan_from_chapter`——它会自动把书从完结态解冻回写作态。不要因为"已完结"就拒绝用户的重写诉求。

### 全书完成

writer commit 返回 `book_complete=true` 后 Host 不再派发。请输出全书总结（总章数 / 总字数 / 各章概要 / 主要角色弧线 / 伏笔回收）后正常结束。

完结后若用户再次干预要求重写整本书，按上面"全书重规划"确认门处理——replan 能把已完结的书解冻重写。
